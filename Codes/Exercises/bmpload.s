...
MAIN: nop
    li a7, 101
    la a0, teste
    jal ra, PRINTA_TELA_WORD

    j EXIT
####################  PRINTA_SPRITE
## Precisa do BTMPi e do BTMPf;
# a0 -> end.
PRINTA_TELA_WORD: nop
    addi sp, sp, -12
    sw s0, 0(sp)
    sw s1, 4(sp)
    sw s2, 8(sp)
    ...
    beq t4, zero, PRINTA_TELA_WORD_EXIT

PRINTA_TELA_WORD_LOOP: nop
    bge t0, t1, PRINTA_TELA_WORD_EXIT
    lw t2, 0(s0)            # Ler uma word do vetor do arquivo
    sw t2, 0(t0)            # Grava uma word na mem VGA
    addi s0, s0, 4
    addi t0, t0, 4
    j PRINTA_TELA_WORD_LOOP

PRINTA_TELA_WORD_EXIT: nop
    lw s0, 0(sp)
    lw s1, 4(sp)
    lw s2, 8(sp)
    addi sp, sp, 12
