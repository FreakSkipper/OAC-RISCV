...
# v1.1 2019/1 by
# Alexandre Souza Costa Oliveira - 17/0033813
...
# tabela de excecoes
#0 : Instruction address misaligned – endereço da instrução desalinhado
#1 : Instruction access fault - endereço fora do segmento .text
#2 : Ilegal Instruction – Instrução não reconhecida
#4 : Load address misaligned – endereço de load desalinhado (obs.: lw, lh, lhu)
#5 : Load access fault – endereço fora do segment .data
#6 : Store address misaligned – endereço de store desalinhado (obs.: sw, sh)
#7 : Store access fault – endereço fora do segment .data
...
StoreAccessFault:	.string "Error: 7 Store acess fault"
StringPC:			.string "PC:"
...
exceptionHandling:  #j ecallException
					csrr tp, 66	# le a causa da exceção
					
					# UCAUSE = 0 : Instruction address misaligned – endereço da instrução desalinhado
					beq tp, zero, instrMisalignedException
					
					# UCAUSE = 1 : Instruction access fault - endereço fora do segmento .text
					addi tp, tp, -1	# ( se tp for = 1, ao subtrair por 1, será tp = 0 ?)
					beq tp, zero, instrAcessFaultException
					
					# UCAUSE = 2 : Ilegal Instruction – Instrução não reconhecida
					addi tp, tp, -1	# ao subtrair novamente, encontrará tp = zero ?
					beq tp, zero, instrIlegalException
					
					# UCAUSE 4 : Load address misaligned – endereço de load desalinhado (obs.: lw, lh, lhu)
					addi tp, tp, -2	# ao subtrair por 2, encontrará tp = zero ?
					beq tp, zero, loadMisalignedException
					
					# UCAUSE 5 : Load access fault – endereço fora do segment .data
					addi tp, tp, -1	# ao subtrair novamente, encontrará tp = zero ?
					beq tp, zero, loadAccessFaultException
					
					# UCAUSE 6 : Store address misaligned – endereço de store desalinhado (obs.: sw, sh) 
					addi tp, tp, -1	# ao subtrair novamente, encontrará tp = zero ?
					beq tp, zero, storeMisalignedException

					# UCAUSE 7 : Store access fault – endereço fora do segment .data
					addi tp, tp, -1	# ao subtrair novamente, encontrará tp = zero ?
					beq tp, zero, storeAcessFaultException										
					
					## ecall ##
					j ecallException		# Por enquanto somente a exceção de ecall
